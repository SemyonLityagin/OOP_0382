...
private:
    ...
    R3 ruleThing;
public:
    Game(R1 rule1, R2 rule2, R3 rule3) : ruleBuilder(rule1), ruleEminy(rule2), ruleThing(rule3){};
    void StartGame(){
        //Music background;
        //background.setLoop(true);
        //background.openFromFile("../audio/Game.wav");
        //background.play();
        if((ruleEminy.GetValue() + ruleThing.GetValue()) >= (Size-2*Size/3)*(Size-2*Size/3)){
            ruleEminy.SetValue(10);
            ruleThing.SetValue(5);
        }
        //SoundBuffer sound;
        ...
        //create enemy
        Unit** evil = CreateEvil(field->GetCells(), ruleEminy.GetValue());
        //create things
        ...
        Move::Movement(field, gamer, evil, thing, ruleEminy.GetValue(), ruleThing.GetValue(), logger);

        delete field;
        for(int i = 0; i < ruleEminy.GetValue(); i++){
            delete evil[i];
        }
        delete [] evil;
